...
class ConfessionShareSheet extends StatefulWidget {
  ...
  static void show(BuildContext context, Confession confession) {
    showModalBottomSheet(
      ...
      builder: (_) => ConfessionShareSheet(confession: confession),
    );
  }
...
class _ConfessionShareSheetState extends State<ConfessionShareSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<_P> items = [
      ...
      _P('WhatsApp', const Color(0xFF25D366), Icons.chat_bubble_rounded,
          () => _shareTo('https://wa.me/?text=$_encoded')),
      _P('Facebook', const Color(0xFF1877F2), Icons.facebook_rounded,
          () => _shareTo('https://www.facebook.com/sharer/sharer.php?u=$_encodedLink')),
      _P('Instagram', const Color(0xFFE1306C), Icons.camera_alt_rounded,
          () { Navigator.pop(context); Share.share(_shareText); }),
      _P('Telegram', const Color(0xFF229ED9), Icons.send_rounded,
          () => _shareTo('https://t.me/share/url?url=$_encodedLink&text=$_encoded')),
      _P('Twitter/X', const Color(0xFF1DA1F2), Icons.alternate_email_rounded,
          () => _shareTo('https://twitter.com/intent/tweet?text=$_encoded')),
      _P('More', AppColors.textSecondary, Icons.more_horiz_rounded,
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) => _PlatformIcon(p: items[i]),
              ),
            ),
          ],
        ),
...
class _P {
  ...
  const _P(this.label, this.color, this.icon, this.onTap);
}

class _PlatformIcon extends StatelessWidget {
  final _P p;
  const _PlatformIcon({required this.p});
